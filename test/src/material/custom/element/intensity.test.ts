...
describe('random drop intensity', () => {
    const ARBITRARY_NUMBER_RANDOM_MIGHT_HAVE_GENERATED: number = 0.3
    it('when the fading intensity is less than the randomly generated value, the note is dropped (intensity set to 0)', () => {
    ...
    it('when the fading intensity is greater than the randomly generated value, the note is fully present (intensity set to 1)', () => {
        const randomSpy: Spy = jasmine.createSpy()
        randomSpy.and.returnValue(ARBITRARY_NUMBER_RANDOM_MIGHT_HAVE_GENERATED)
